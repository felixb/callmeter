...
public class PreferencesRules extends SherlockPreferenceActivity {
	...
	private static class OnRuleClickListener implements OnPreferenceClickListener {
		...
		public boolean onPreferenceClick(final Preference preference) {
			try {
				final String p = this.j.getString("provider").trim();
				final String t = this.j.getString("title").trim();
				final String d = this.j.isNull("description") ? null : this.j.getString(
						"description").trim();
				final String ld = this.j.isNull("longdescription") ? null : this.j.getString(
						"longdescription").trim();
				final String link = this.j.isNull("link") ? null : this.j.getString("link").trim();

				final String url = this.j.getString("importurl").trim();
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		String k = this.getIntent().getStringExtra(EXTRA_COUNTRY);
		this.setTitle(k);

		this.addPreferencesFromResource(R.xml.group_prefs);
		PreferenceScreen base = (PreferenceScreen) this.findPreference("container");
