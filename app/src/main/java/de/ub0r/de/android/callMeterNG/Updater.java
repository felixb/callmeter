...
class Updater extends AsyncTask<Void, Void, Integer[]> {
     ...
     *
     * @param p {@link SharedPreferences} to read plans from
     ...
     * @param idNumber id of number in cursor
     * @param plans    plans
     * @param toPlan   bill excluded numbers in this plan, 0 for no plan.
     ...
     * @param calBillDate Date of billdate
    ...
    private void walkCalls(final boolean[][] plans, final Calendar calBillDate,
        ...
        if (this.prefs.getBoolean(PREFS_CALLS_BILL_INCOMING, false)) {
            ...
            if (status[RESULT_CALLS1_LIMIT] > 0) {
                this.callsInOut1 = (sum * CallMeter.HUNDRED / // .
            ...
            if (status[RESULT_CALLS2_LIMIT] > 0) {
                this.callsInOut2 = (sum * CallMeter.HUNDRED / // .
    ...
    private void walkSMS(final boolean[][] plans, final Calendar calBillDate,
        ...
        if (this.prefs.getBoolean(PREFS_SMS_BILL_INCOMING, false)) {
            ...
            if (free1 > 0) {
                this.smsInOut1 = (sum * CallMeter.HUNDRED / free1) + "% / ";
            } else {
                this.smsInOut1 = "";
            }
            this.smsInOut1 += sum;
            ...
            if (free2 > 0) {
                this.smsInOut2 = (sum * CallMeter.HUNDRED / free2) + "% / ";
            } else {
                this.smsInOut2 = "";
            }
            this.smsInOut2 += sum;
        ...
        if (this.prefs.getBoolean(PREFS_MERGE_SMS_TO_CALLS, false)) {
            ...
            if (this.prefs.getBoolean(PREFS_CALLS_BILL_INCOMING, false)) {
                ...
                if (freesum > 0) {
                    ssum = (sum * CallMeter.HUNDRED / freesum) + "% / ";
                } else {
                    ssum = "";
                }
                ssum += getTime(sum);
     ...
     *
     * @param thisPeriod used this billperiod
     * @param limit      limit
     * @param all        used all together
     * @param calls      calls/sms?
    ...
    private static String calcString(final int thisPeriod, final int limit,
        ...
        if (limit > 0) {
            if (calls) {
                return ((thisPeriod * CallMeter.HUNDRED) / // .
                        (limit * SECONDS_MINUTE))
                        + "% | " + getTime(thisPeriod) + c
                        + " | "
                        + getTime(all);
            } else {
                return ((thisPeriod * CallMeter.HUNDRED) / limit) + "% | "
                        + thisPeriod + " | " + all;
            }
        } else {
            if (calls) {
     ...
     *
     * @param billDay first day of bill
