...
/**
 * AsyncTask to handle calculations/display for data in background.
 *
 * @author flx
 */
class UpdaterData extends AsyncTask<Void, Void, Long[]> {
     ...
     * Tag for output.
     */
    private static final String TAG = "UpdaterData";
    ...
    protected final void onPostExecute(final Long[] result) {
        if (this.updateGUI) {
            ...
            if (result[1] > 0) {
                ...
                String s = ((result[0] * CallMeter.HUNDRED) / result[1]) + "%";
                if (cost != null) {
                    s = cost + " | " + s;
                }
                if (onlyIn) {
     ...
     */
    static Calendar getBillDate(final SharedPreferences p) {
     ...
     */
    static synchronized void updateTraffic(final Context context,
     ...
     * @param prefs preferences
     */
    static void checkBillperiod(final SharedPreferences prefs) {
     ...
     * @param prefs preferences
     */
    static void checkPostboot(final SharedPreferences prefs) {
        long preBootIn = prefs.getLong(PREFS_DATA_BOOT_IN, 0);
        long preBootOut = prefs.getLong(PREFS_DATA_BOOT_OUT, 0);
        long runningIn = prefs.getLong(PREFS_DATA_RUNNING_IN, 0);
        long runningOut = prefs.getLong(PREFS_DATA_RUNNING_OUT, 0);
